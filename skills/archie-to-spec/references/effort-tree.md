...
One directory per Effort, nested under `.archie/`. The filesystem is the tree, so there are no parent pointers to maintain and no orphans.

```
.archie/new-web-app/            root Effort, unnumbered
├── effort.md
...
Only `NN-<slug>` directories are children. `research/` and `prototypes/` are session artifacts written by `/archie-research` and `/archie-prototype`, and an Effort carrying them is still thin.
