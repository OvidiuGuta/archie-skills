---
name: interview
description: Interviewing — one question a turn, gated by the altitude test, with deferrals clustering into child Efforts. Reached by /architect while scoping an Effort.
---
...
Read [`../reference/altitude.md`](../reference/altitude.md) first. It fixes the gate this skill enforces and names the exit a hard question takes.
...
When the answer lands, record it before asking the next question — [`../reference/decisions.md`](../reference/decisions.md) settles where it goes. The Effort tree is disposable, so a decision left in the conversation goes when the tree goes.

## A hard question is not a low one

An at-altitude question that conversation resolves badly stays in the session and takes another exit: a **fact** the user would have to go and read goes to `/research`, and a "how should this look" goes to `/prototype`. Hand it to `/architect`, which dispatches the sub-agent, and carry on with the frontier while it runs.
...
The session ends when the frontier is empty. Report to `/architect` in one message:

- **An empty frontier and no deferrals** — this Effort is understood at its own resolution. `/architect` weighs specifying it.
...
The recommendation on splitting or specifying is `/architect`'s, and the call is the user's.
