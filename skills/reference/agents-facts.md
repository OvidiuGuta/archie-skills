...
`/setup-archie` seeds the section. Any skill that later discovers a fact has changed, or creates a gate itself, writes the correction back — `/implement`'s orchestrator runs the gates every run, so it is the natural healing point.
...
- `/qa` reading an unknown run command asks the user rather than trying `npm start`.
- `/qa` blocks E2E on unknown seed data or credentials, and reports the affected criteria as unverified.
- `/tdd` reading an unknown gate command asks for it, then writes the answer back into the section.
