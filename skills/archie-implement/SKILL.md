...
No `.archie/` at all is a repo that has never been planned. Say so and name `/archie-architect`, then `/archie-to-spec` and `/archie-to-tasks`, rather than inventing a Task from the reference. Implementing installs without Planning, so this is a state you will meet.
