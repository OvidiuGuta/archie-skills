---
name: archie-tdd
description: Building one Task test-first in a double loop — one failing integration test at the spec's seam, then unit tests driving each unit the Task modifies — and running the repo's gates. The engineer step of /archie-implement's ready-for-agent pipeline.
---
...
Read [`references/templates/design.md`](./references/templates/design.md) first: it fixes what the design file you are handed contains. The whole-app journey test is `/archie-qa`'s, not yours.
...
- The **test prior art** named in the [facts section](./references/agents-facts.md), and the code the Task lands in. New tests match the house style rather than importing yours.
...
Assert **behaviour at the unit's boundary**: what it returns, what it emits, what it calls on its collaborators. A test that would break when a symbol is renamed or a helper is extracted is testing implementation, and `/archie-code-review` rejects it.
...
Run the lint, typecheck, test and build commands recorded in the [facts section](./references/agents-facts.md), exactly as written there. Green tests with a red typecheck is a broken Task.
