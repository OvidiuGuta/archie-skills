---
name: research
description: Resolving a factual question in a sub-agent that writes its findings to a file and returns the path. Reached by /architect when a question in a session is a fact rather than a decision.
---
...
- **The destination**: `research/<slug>.md` inside the [Effort's directory](../reference/effort-tree.md). It is committed and disposable with the rest of the tree, so anything that must outlive the tree reaches `CONTEXT.md` or `docs/adr/` through `/domain-modeling`.
...
```md
_Researched:_ {the question} → `.scratch/{effort path}/research/{slug}.md`
{The answer, in one or two lines, and what is still unsettled.}
```

`/architect` reads those two lines, asks the question that was waiting on them, and opens the file only if the answer turns out to be contested.
