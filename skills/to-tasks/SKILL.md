---
name: to-tasks
description: Slicing one Effort's spec.md into the Tasks /implement consumes — vertical tracer bullets, one demoable outcome each, sequenced by their blocking edges. Run it once the Spec is written.
disable-model-invocation: true
---

# To tasks

The Spec says what the leaf is. This cuts it into the Tasks `/implement` runs one at a time, unattended. The cut is the last thing a human shapes: from here the pipeline builds whatever these files say, so a Task that hides two outcomes is two half-built features nobody demoed.

Read [`../reference/templates/task.md`](../reference/templates/task.md) first. It fixes the file's shape and the rules governing it. [`../reference/effort-tree.md`](../reference/effort-tree.md) carries the numbering, the statuses and the two labels.

You write task files and nothing else. Their designs are `/implement`'s to produce.
...
- **Blocking edges.** A Task blocks another when the second genuinely cannot start until the first lands. Edges are the schedule; anything unblocked can run whenever `/implement` reaches it.
...
Report the leaf's path, the Task count, and the first Task by reference (`3.2#1`). Name `/implement` as the next move and stop. The Effort is now sliced, and everything after this runs AFK.
