---
name: code-review
description: Reviewing one Task's diff against its three contracts — the Spec, the Task's acceptance criteria, and the design.md it claims to follow — plus the Fowler smell baseline, and returning triaged findings. The reviewer step of /implement's ready-for-agent pipeline.
---
...
- The **task file** — the demoable outcome and the [acceptance criteria](../reference/templates/task.md), which are the contract the outcome is judged against.
- The **`design.md`** beside it — the modules, the seam, and the [units to pin](../reference/templates/design.md). This is what the code claims to have done.
- The leaf's **`spec.md`** — the [seam](../reference/templates/spec.md) and the Implementation Decisions.
- The **test prior art** named in the [facts section](../reference/agents-facts.md), so house style is what you measure against rather than your own.
...
The [three test layers](../../docs/adr/0006-three-test-layers-split-by-altitude.md) are what makes the outcome hold tomorrow, so they are reviewed as carefully as the code:
...
- **Every smell is a judgement call**, labelled as one ("possible Feature Envy") rather than reported as a breach, and it never blocks. Skip anything the repo's lint and typecheck gates already enforce, since those ran in `/tdd`.
...
- **Planning defect** — the contracts themselves are the problem: an ambiguous or unbuildable acceptance criterion, code that is right against the Spec and wrong against the design, or two contracts that contradict each other. Its blast radius reaches past this Task, so only the user settles it. See [`../reference/altitude.md`](../reference/altitude.md).
