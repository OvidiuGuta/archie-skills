---
name: qa
description: Proving one Task's demoable outcome against the running app — one Playwright journey test walking it front to back — and grading every acceptance criterion as pinned, verified but not pinned, or unverified. The QA step of both of /implement's pipelines.
---
...
- The **task file** — the [demoable outcome](../reference/templates/task.md) is the journey, and the acceptance criteria are what you grade. Its **label** decides your run: `ready-for-agent` walks every step below, `ready-for-human` skips step 3.
...
- The [facts section](../reference/agents-facts.md) — the run command and its port, the seed data and test credentials, the **E2E harness** and where its specs live. That directory is your prior art: a new journey matches the house style there.
...
A criterion the app **contradicts** is a finding rather than a grade. Label it as [`/code-review`](../code-review/SKILL.md) does: a **code defect** when the contracts are sound and the app missed them, a **planning defect** when the criterion itself cannot be built or checked as written, whose blast radius reaches past this Task and only the user settles. See [`../reference/altitude.md`](../reference/altitude.md).
