...
description: Designing how one Task is built in this codebase, in a read-only sub-agent that writes tasks/NN-<slug>.design.md and returns the path. The first step of /implement's ready-for-agent pipeline.
---
...
Read [`../reference/templates/design.md`](../reference/templates/design.md) first: it fixes the file's shape and the rules governing it.
...
Read the code this Task lands in before designing anything for it, and read the [test prior art](../reference/agents-facts.md) the new tests will sit beside. The design's job is to build with the codebase's **grain** — its existing module boundaries, its naming, its layering — so name the real files and symbols you found and say which are new and which change.
...
The Spec settled what spans the leaf. You settle what lives inside this Task: the modules and components it adds or changes, what each owns and exposes, the units to pin and the behaviour to assert at each boundary, and the ordered sequence the engineer works through. Anything whose blast radius reaches past this Task is the Spec's, already decided — see [`../reference/altitude.md`](../reference/altitude.md). Designing the whole leaf here is the failure mode: it commits the Tasks after this one before their siblings' code exists.
...
```md
_Designed:_ {Task reference} — {Task title} → `.scratch/{effort path}/tasks/{NN}-{slug}.design.md`
{The approach in one or two lines, and the seam it attaches at.}
```

Never the design inline. The orchestrator hands the path to `/tdd` and never needs to read the file itself.
