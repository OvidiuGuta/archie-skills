---
name: prototype
description: Building a throwaway artifact in a sub-agent for the user to react to, and revising it in that same sub-agent. Reached by /architect when a question is "how should this look or behave".
---
...
```md
_Prototype:_ {the question it answers} → `.scratch/{effort path}/prototypes/{slug}/`
{How to look at it, in one line.}
```

The reaction happens at the top level, in the user's own words, and `/architect` records what it settles through `/domain-modeling`. That is the whole product of the exercise; the directory is the receipt.
