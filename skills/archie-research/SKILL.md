...
- **The destination**: `research/<slug>.md` inside the Epic's own directory under `.archie/`. It is committed and disposable with the rest of the tree, so anything that must outlive the tree reaches `CONTEXT.md` or `docs/adr/` through `/archie-domain-modeling`.
...
```md
_Researched:_ {the question} → `.archie/{epic path}/research/{slug}.md`
