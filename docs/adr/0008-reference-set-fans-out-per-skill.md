...
- A skill needing one fact from a long reference file should **inline the fact** instead. `/archie-prototype` and `/archie-research` each pulled in 88 lines of `effort-tree.md` for a single path, and now carry no reference copies at all.
